...
if __name__ == "__main__":
    ...
    likelihood = likelihood[50:155, 50:150]  # crop a piece
    groundtruth = groundtruth[50:150, 50:150] # approx gt using likelihood
    ...
    # Compute the Betti number for binary groundtruth image
    # width1, height1 = groundtruth.shape
    # imagegray = []
    # for i in range(width1):
    #     for j in range(height1):
    #         if groundtruth[i,j]> 0.5:
    #             imagegray.append([i, j])
    # s = np.array((imagegray))
    # print(type(imagegray))
    # grid = hi.compute_distance_to_a_measure(s, [width1, height1], nn=1)
    # temp = gdh.compute_persistence_diagram(grid, i=0)
    temp = gdh.compute_persistence_diagram(groundtruth, i=0)
    pd_gt = np.array([[0,255],]*len(temp))
