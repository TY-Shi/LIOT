...
from pre_processing import my_PreProc, my_augment, my_PreProc_three
...
def get_data_training(DRIVE_train_imgs_original,
    ...
    #without augment
    train_imgs, train_masks = my_augment(train_imgs_original,train_masks)
...
def get_data_testing(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width):
    ...
    test_imgs = my_PreProc_three(test_imgs_original)
...
def get_singledata_testing_overlap(Single_img, Single_gt, patch_height, patch_width,
                             stride_height, stride_width):
    ### test
    test_imgs_original = np.expand_dims(Single_img,axis=0)
    test_masks = np.expand_dims(Single_gt,axis=0)
...
def get_singledata_testing_overlap_1(Single_img, Single_gt, patch_height, patch_width,
                             stride_height, stride_width):
    ### test
    Single_img_1 = Single_img[1,:,:]
    Single_img_1 = np.expand_dims(Single_img_1, axis=0)
    test_imgs_original = np.expand_dims(Single_img_1,axis=0)
    test_masks = np.expand_dims(Single_gt,axis=0)
...
def get_singledata_testing_overlap_onlyimg(Single_img, patch_height, patch_width,
                             stride_height, stride_width):
    ### test
    Single_img_1 = Single_img
    test_imgs_original = np.expand_dims(Single_img_1,axis=0)
...
def data_consistency_check(imgs, masks):
    ...
    assert (imgs.shape[1] == 1 or imgs.shape[1] == 3 or imgs.shape[1] == 4)
...
def extract_random(full_imgs, full_masks, patch_h, patch_w, N_patches, inside=True):
    ...
    assert (full_imgs.shape[1] == 1 or full_imgs.shape[1] == 3 or full_imgs.shape[1] == 4)  # check the channel is 1 or 3
...
# extract patches randomly in the full training images
#  -- Inside OR in full image
def extract_random_img(full_imgs, full_masks, patch_h, patch_w):
    assert (len(full_imgs.shape) == 3 and len(full_masks.shape) == 3)  # 4D arrays
    assert (full_imgs.shape[0] == 1 or full_imgs.shape[0] == 3 or full_imgs.shape[0] == 4 or full_imgs.shape[0] == 7)  # check the channel is 1 or 3
    assert (full_masks.shape[0] == 1)  # masks only black and white
    assert (full_imgs.shape[1] == full_masks.shape[1] and full_imgs.shape[2] == full_masks.shape[2])
    patches = np.empty((full_imgs.shape[0], patch_h, patch_w))
    patches_masks = np.empty((full_masks.shape[0], patch_h, patch_w))
    img_h = full_imgs.shape[1]  # height of the full image
    img_w = full_imgs.shape[2]  # width of the full image
    x_center = random.randint(0 + int(patch_w / 2), img_w - int(patch_w / 2))
    y_center = random.randint(0 + int(patch_h / 2), img_h - int(patch_h / 2))
    patch = full_imgs[:, y_center - int(patch_h / 2):y_center + int(patch_h / 2),
            x_center - int(patch_w / 2):x_center + int(patch_w / 2)]
    patch_mask = full_masks[:, y_center - int(patch_h / 2):y_center + int(patch_h / 2),
                 x_center - int(patch_w / 2):x_center + int(patch_w / 2)]
...
def paint_border_overlap(full_imgs, patch_h, patch_w, stride_h, stride_w):
    assert (len(full_imgs.shape) == 4)  # 4D arrays
    assert (full_imgs.shape[1] == 1 or full_imgs.shape[1] == 3 or full_imgs.shape[1] == 4 or full_imgs.shape[1] == 7)  # check the channel is 1 or 3
    img_h = full_imgs.shape[2]  # height of the full image
    img_w = full_imgs.shape[3]  # width of the full image
...
def recompone_overlap(preds, img_h, img_w, stride_h, stride_w):
    ...
    assert (preds.shape[1] == 1 or preds.shape[1] == 3 or preds.shape[1] == 4)  # check the channel is 1 or 3
...
def recompone(data, N_h, N_w):
    assert (data.shape[1] == 1 or data.shape[1] == 3 or data.shape[1] == 4)  # check the channel is 1 or 3
    ...
    for i in range(data_imgs.shape[0]):  # loop over the full images
        for x in range(width):
            for y in range(height):
                if original_imgs_border_masks!=None and inside_FOV_DRIVE(i, x, y, original_imgs_border_masks) == True:
...
def pred_only_FOV_list(data_imgs_list, data_masks_list, original_imgs_border_masks_list,Test_Dataset):
    new_pred_imgs = []
    new_pred_masks = []
    for idx in range(0, len(data_imgs_list)):
        data_imgs = data_imgs_list[idx]
        data_masks = data_masks_list[idx]
        original_imgs_border_masks = original_imgs_border_masks_list[idx]
        assert (len(data_imgs.shape) == 4 and len(data_masks.shape) == 4)  # 4D arrays
        assert (data_imgs.shape[0] == data_masks.shape[0])
        assert (data_imgs.shape[2] == data_masks.shape[2])
        assert (data_imgs.shape[3] == data_masks.shape[3])
        ...
        for i in range(data_imgs.shape[0]):  # loop over the full images
            for x in range(width):
                for y in range(height):
                    if Test_Dataset!='Crack' and Test_Dataset.find('Crack')==-1 and inside_FOV_DRIVE(i, x, y, original_imgs_border_masks) == True:
                        new_pred_imgs.append(data_imgs[i, :, y, x])
                        new_pred_masks.append(data_masks[i, :, y, x])
                    elif Test_Dataset=='Crack' or Test_Dataset.find('Crack')!=-1:
                        new_pred_imgs.append(data_imgs[i, :, y, x])
                        new_pred_masks.append(data_masks[i, :, y, x])
